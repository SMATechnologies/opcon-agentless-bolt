...
public class RunCommandJobDetailsAction implements IJobDetailsAction {
	...
	private static final String COMMAND_TOOLTIP = "Command to run, wrap shell special characters in single quotation marks";
	...
	public RunCommandJobDetailsAction(IPersistentJob job, ContextID contextID, IMessageDisplayer messageDisplayer) {
		this._job = job;
	...
	public void createActionComposite(Composite jobActionComposite) {
		...
		_commandText = JobUtil.createLabeledText(commandGroup, COMMAND_NAME, 0, JobUtil.COLOR_BLUE, JobUtil.COLOR_LIGHT_GREEN, 
				SWT.BORDER, 4);
		_commandText.setToolTipText(COMMAND_TOOLTIP);
	}
	...
	public void addListeners(IDirtyListener listener) {
		...
		_commandText.addModifyListener(new DirtyModifyAdapter(listener));
		...
		new ControlTokenSelectorListener(_commandText, _contextID);
	}
	...
	public void setDefaults() {
		...
		_commandText.setText(SystemConstants.EMPTY_STRING);
	}
	...
	public void initializeContents(AgentlessBoltData agentlessBoltData) {
		...
		_commandText.setText(agentlessBoltData.getCommand());
	}
	...
	public AgentlessBoltData getContents() {
		...
		data.setBoltTask(AgentlessBoltEnums.Task.command);
		...
		data.setCommand(_commandText.getText());
		return data;
	}
