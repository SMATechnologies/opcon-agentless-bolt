...
public class RunScriptJobDetailsAction implements IJobDetailsAction {
	...
	private static final String SCRIPT_TOOLTIP = "Script to execute on remote system";
	private static final String ARGUMENTS_NAME = "Arguments";
	private static final String ARGUMENTS_TOOLTIP = "Arguments associated with the script";
	...
	private Text _argumentText;
	
	public RunScriptJobDetailsAction(IPersistentJob job, ContextID contextID, IMessageDisplayer messageDisplayer) {
		this._job = job;
	...
	public void createActionComposite(Composite jobActionComposite) {
		...
		// command
		Group scriptGroup = JobDetailsHelper.createGroup(_composite, SystemConstants.EMPTY_STRING, 2, false);
		scriptGroup.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, true));

		_scriptText = JobUtil.createLabeledText(scriptGroup, SCRIPT_NAME, 0, JobUtil.COLOR_BLUE, JobUtil.COLOR_LIGHT_GREEN, SWT.BORDER, 1);
		_scriptText.setToolTipText(SCRIPT_TOOLTIP);

		_argumentText = JobUtil.createLabeledText(scriptGroup, ARGUMENTS_NAME, 0, JobUtil.COLOR_BLUE, JobUtil.COLOR_LIGHT_GREEN, SWT.BORDER, 2);
		_argumentText.setToolTipText(ARGUMENTS_TOOLTIP);
	}
	...
	public void addListeners(IDirtyListener listener) {
		...
		_scriptText.addModifyListener(new DirtyModifyAdapter(listener));
		_argumentText.addModifyListener(new DirtyModifyAdapter(listener));
		...
		new ControlTokenSelectorListener(_scriptText, _contextID);
		new ControlTokenSelectorListener(_argumentText, _contextID);
	}
	...
	public void setDefaults() {
		...
		_scriptText.setText(SystemConstants.EMPTY_STRING);
		_argumentText.setText(SystemConstants.EMPTY_STRING);
	}
	...
	public void initializeContents(AgentlessBoltData agentlessBoltData) {
		...
		_scriptText.setText(agentlessBoltData.getScript());
		if(agentlessBoltData.getArgument() != null) {
			_argumentText.setText(agentlessBoltData.getArgument());
		} else {
			_argumentText.setText(SystemConstants.EMPTY_STRING);
		}
	}
	...
	public AgentlessBoltData getContents() {
		...
		data.setBoltTask(AgentlessBoltEnums.Task.script);
		...
		data.setScript(_scriptText.getText());
		if(_argumentText.getText().length() > 0) {
			data.setArgument(_argumentText.getText());
		}
		return data;
	}
