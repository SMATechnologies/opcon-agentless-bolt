...
public class AgentlessBoltWindowsJobDetailsWidget extends AbstractWindowsSubJobDetailsWidget {
	...
	public AgentlessBoltWindowsJobDetailsWidget(Composite parent, IMessageDisplayer messageManager, ContextID context, IJobDetailEditorContext iJobDetailEditorContext) {
	...
	private void loadTaskActionWidget(IJobDetailsAction toLoad) {
		...
		_currentTaskAction.addListeners(AgentlessBoltWindowsJobDetailsWidget.this);
	...
	protected String getWorkingDirectory() {
		return JobUtil.autoQuote(_locPathText.getText() + SystemConstants.BACK_SLASH, true);
	}
	...
	protected void initializeContents(ISpecificJobProperties jobProperties)
		...
		try {
			if(windowsDetails != null) {
				...
				_agentlessBoltData = parseCommandLine(commandLine);
				if(_agentlessBoltData.getBoltTask() == AgentlessBoltEnums.Task.command) {
					...
					loadTaskActionWidget(new RunScriptJobDetailsAction(getInput(), getContextID(), getMessageDisplayer()));
				} else if(_agentlessBoltData.getBoltTask() == AgentlessBoltEnums.Task.task) {
					loadTaskActionWidget(new RunTaskJobDetailsAction(getInput(), getContextID(), getMessageDisplayer()));
				}
	...
	private AgentlessBoltData parseCommandLine(String commandLine) throws ParseException {
		...
		// remove 
		if(commandLine.contains(AgentlessBoltConstants.CommandLineArguments.TargetNoHostKeyCheck)) {
			commandLine = commandLine.replace(AgentlessBoltConstants.CommandLineArguments.TargetNoHostKeyCheck, SystemConstants.EMPTY_STRING);
			_agentlessBoltData.setNoHostKeyCheck(true);
		}
		// remove executable stuff
		location = commandLine.indexOf(COMMAND_SUFFIX);
		commandLine = commandLine.substring(location + COMMAND_SUFFIX.length() + 2, commandLine.length()).trim();
		// parse from the end
		if(commandLine.contains(CommandLineArguments.SudoPasswordArgument)) {
			location = commandLine.indexOf(AgentlessBoltConstants.CommandLineArguments.SudoPasswordArgument);
			String sudoPassword = commandLine.substring(location, commandLine.length()).trim();
			...
			commandLine = commandLine.substring(0, location).trim();
		}
		if(commandLine.contains(CommandLineArguments.RunAsArgument)) {
			location = commandLine.indexOf(AgentlessBoltConstants.CommandLineArguments.RunAsArgument);
			String runAs = commandLine.substring(location, commandLine.length()).trim();
		...
		location = commandLine.indexOf(AgentlessBoltConstants.CommandLineArguments.PasswordArgument);
		String password = commandLine.substring(location, commandLine.length()).trim();
		...
		commandLine = commandLine.substring(0, location).trim();
		location = commandLine.indexOf(AgentlessBoltConstants.CommandLineArguments.UserArgument);
		String user = commandLine.substring(location, commandLine.length()).trim();
		...
		commandLine = commandLine.substring(0, location).trim();
		if(commandLine.contains(AgentlessBoltConstants.CommandLineArguments.TargetNoSSL)) {
			commandLine = commandLine.replace(AgentlessBoltConstants.CommandLineArguments.TargetNoSSL, SystemConstants.EMPTY_STRING).trim();
			_agentlessBoltData.setSsl(false);
		} else {
			_agentlessBoltData.setSsl(true);
		}
		location = commandLine.indexOf(AgentlessBoltConstants.CommandLineArguments.TargetsArgument);
		String targets = commandLine.substring(location, commandLine.length()).trim();
		...
		commandLine = commandLine.substring(0, location).trim();
		if(commandLine.contains(AgentlessBoltConstants.CommandLineArguments.FileUpload)) {
			// extract file upload data
			commandLine = commandLine.replace(AgentlessBoltConstants.CommandLineArguments.FileUpload, SystemConstants.EMPTY_STRING).trim();
			String[] filenames = tokenizeParameters(commandLine, false, "' '");
		...
		} else if(commandLine.contains(AgentlessBoltConstants.CommandLineArguments.CommandRun)) {
			// extract command run data
			commandLine = commandLine.replace(AgentlessBoltConstants.CommandLineArguments.CommandRun, SystemConstants.EMPTY_STRING).trim();
			if(commandLine.startsWith(SystemConstants.QUOTE_SINGLE)) {
				commandLine = removeLeadingTrailingSingleQuotes(commandLine);
				_agentlessBoltData.setCommand(commandLine);
			} else {
			...
			_taskItemCombo.setSelection(AgentlessBoltEnums.Task.command, true);
			_taskItemCombo.setEnabled(false);
		} else if(commandLine.contains(AgentlessBoltConstants.CommandLineArguments.ScriptRun)) {
			// extract script run data
			commandLine = commandLine.replace(AgentlessBoltConstants.CommandLineArguments.ScriptRun, SystemConstants.EMPTY_STRING).trim();
			String[] arguments = tokenizeParameters(commandLine, false, "' '");
			...
			_taskItemCombo.setEnabled(false);
		} else if(commandLine.contains(AgentlessBoltConstants.CommandLineArguments.TaskRun)) {
			// extract task run data
			commandLine = commandLine.replace(AgentlessBoltConstants.CommandLineArguments.TaskRun, SystemConstants.EMPTY_STRING).trim();
			String[] arguments = tokenizeParameters(commandLine, false, "' '");
